...
- **Tauri + システムWebView2。** Chromium、Node.js、FFmpegをクライアントに同梱しません。ZIPは約1.95 MiBです。
...
2. `SofaMate.exe`を開き、公式テーマやコミュニティ作品をダウンロードするか、自分のH.264/AAC MP4を読み込みます。
3. モニターと画質を選んで「壁紙に設定」を押します。ダウンロード済みの画質を選んだだけでは、デスクトップは切り替わりません。トレイアイコンをダブルクリックするとパネルに戻れます。

Windows 10/11 x64と[Microsoft Edge WebView2 Runtime](https://developer.microsoft.com/microsoft-edge/webview2/)が必要です。複数のモニターに異なる壁紙を設定でき、1本最大1 GiBの動画読み込みに対応します。公開ベータ版は未署名です。動画は同梱していません。自分の動画、[公式テーマやコミュニティ作品](https://sofamate.aveniqa.com/ja/market)をご利用ください。

バナー左は「夜のリビング」の実際のキャラクター、右は新キャラクターのイメージです。右のキャラクターの動画は未公開です。

「夜のリビング」は **720P24・1080P24・1080P60** の完全なテーマパックを提供。4Kは一般向けに配信していません。28本の動画からランダムに開始し、決められた接続先を選んで滑らかに切り替えます。単独の動画は通常のループ再生です。画面ごとに壁紙・画質・表示方式を設定できます。

初めての方もAveniqaの共通ログインでメール認証コード・Google・GitHubによる登録と認証を行えます。[最新の更新内容](docs/RELEASE_NOTES.md)もご覧ください。
...
公開ベータ版です。動画の自動変換、決済、自動アップデートは未対応です。制作アーカイブ、認証情報、ユーザーの投稿動画はリポジトリに含めません。
