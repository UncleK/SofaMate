...
pub fn list(lib:&Library,query:&str,offset:u64)->Result<Value,String>{let mut v=response(client(lib)?.get(format!("{}/v1/items",base(lib))).query(&[("q",query.to_string()),("offset",offset.to_string())]).send())?;
 for item in v["items"].as_array_mut().ok_or("目录无效")?{let item_id=id(item["id"].as_str().ok_or("目录无效")?)?.to_string();
  item["coverUrl"]=json!(format!("{}/v1/items/{}/cover",base(lib),item_id));item["shareUrl"]=json!(format!("{}/wallpaper/{}",base(lib),item_id));}
...
pub fn publish(lib:&Library,app:&tauri::AppHandle,entry_id:&str,title:&str,description:&str,name:&str)->Result<Value,String>{
 ...
 let draft=response(c.post(format!("{}/v1/items",base(lib))).bearer_auth(token).json(&json!({"title":title,"description":description,"bytes":entry.bytes,"sha256":entry.sha256})).send())?;
...
pub fn download(lib:&Library,app:&tauri::AppHandle,item_id:&str)->Result<Entry,String>{
 ...
 let r=c.get(format!("{url}/cover")).send().map_err(|_|"预览图下载失败，可在本机重新生成")?;if !r.status().is_success(){return Err("预览图下载失败，可在本机重新生成".into())}let mut cover=vec![];r.take(1024*1024+1).read_to_end(&mut cover).map_err(|e|e.to_string())?;lib.check_cancel()?;lib.cover(&entry.id,&cover)
}
