...
pub struct Entry{pub id:String,pub title:String,pub bytes:u64,pub sha256:String,pub info:Info,pub created_at:u64,#[serde(default)]pub ready:bool,#[serde(default)]pub market_id:Option<String>}
...
impl Library{
 pub fn open(root:PathBuf,port:u16)->Result<Self,String>{fs::create_dir_all(&root).map_err(|e|e.to_string())?;let mut entries=vec![];
  for dir in fs::read_dir(&root).map_err(|e|e.to_string())?.flatten(){if !dir.file_type().map_err(|e|e.to_string())?.is_dir(){continue}let id=dir.file_name().to_string_lossy().to_string();if !valid_id(&id){continue}
 ...
 pub fn path(&self,id:&str,name:&str)->Result<PathBuf,String>{if !valid_id(id)||!["video.mp4","cover.jpg","entry.json"].contains(&name){return Err("文件标识无效".into())}Ok(self.root.join(id).join(name))}
 ...
 pub fn temp(&self)->Temp{Temp(self.root.join(format!("transfer-{}.tmp",now())))}
 pub fn finish(&self,temp:&Path,title:String,bytes:u64,digest:String,market_id:Option<String>)->Result<Entry,String>{
  ...
  let dir=self.root.join(&digest);fs::create_dir_all(&dir).map_err(|e|e.to_string())?;let dest=dir.join("video.mp4");
  if dest.exists(){return Err("本机文件目录不完整，请重新启动后重试".into())}fs::rename(temp,&dest).map_err(|e|e.to_string())?;
  let entry=Entry{id:digest.clone(),sha256:digest,title,bytes,info,created_at:now(),ready:false,market_id};self.update(entry.clone())?;Ok(entry)
 }
 ...
 pub fn cover(&self,id:&str,bytes:&[u8])->Result<Entry,String>{video::jpeg(bytes)?;let mut entry=self.get(id)?;let dest=self.path(id,"cover.jpg")?;let temp=dest.with_extension("jpg.tmp");fs::write(&temp,bytes).map_err(|e|e.to_string())?;fs::rename(temp,dest).map_err(|e|e.to_string())?;entry.ready=true;self.update(entry.clone())?;Ok(entry)}
